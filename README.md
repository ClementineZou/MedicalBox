...
### 🆘 医疗急救卡
- **关键信息展示**：集中展示血型、过敏史、既往病史、当前用药和紧急联系人
- **ICD-11 疾病搜索**：集成 WHO ICD-11 API，支持中英文搜索添加既往病史，确保医疗信息标准准确
- **自定义病史**：支持手动输入无法搜到的特殊病史
- **打印支持**：专门优化的打印视图，可一键打印为纸质卡片或保存为 PDF 随身携带
- **隐私优先**：仅在用户登录后可见，数据安全存储，可选是否显示器官捐献意愿
...
- **强化隐私保护**：为敏感数据（用药记录、生命体征）提供额外的访问保护，启用后访问敏感页面需进行身份验证
