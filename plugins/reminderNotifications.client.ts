...
export default defineNuxtPlugin((nuxtApp) => {
  ...
  const { isSupported, permission, sendMedicationReminder, areRemindersEnabled } = useBrowserNotifications()
      ...
      reminders.forEach((reminder: any) => {
        ...
        if (reminderTime >= now && reminderTime <= thirtyMinutesLater && !notifiedReminders.has(reminder.id)) {
          ...
          try {
            ...
            if (descData.dosageAmount) {
              dosage = `${descData.dosageAmount}${reminder.medicine?.quantityUnit || '单位'}`
            }
          } catch (e) {
            // 忽略解析错误
          }
          ...
          sendMedicationReminder({
            id: reminder.id,
            title,
            medicineName: reminder.medicine?.name || '药品',
